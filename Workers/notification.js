...
function InitiateNotification( id,tenant, company, message,iss, cb){
    ...
    if((config.Services && config.Services.notificationServiceHost && config.Services.notificationServicePort && config.Services.notificationServiceVersion)) {
        ...
        var notificationURL = format("http://{0}/DVP/API/{1}/NotificationService/Notification/initiate", config.Services.notificationServiceHost, config.Services.notificationServiceVersion);
        if (validator.isIP(config.Services.notificationServiceHost))
            notificationURL = format("http://{0}:{1}/DVP/API/{2}/NotificationService/Notification/initiate", config.Services.notificationServiceHost, config.Services.notificationServicePort, config.Services.notificationServiceVersion);
